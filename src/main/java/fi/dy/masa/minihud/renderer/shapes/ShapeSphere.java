...
public class ShapeSphere extends ShapeBase
{
    ...
    public ShapeSphere()
    {
        super(ShapeTypes.SPHERE, Configs.Colors.DESPAWN_SPHERE_OVERLAY_COLOR.getColor());
    ...
    public void fromJson(JsonObject obj)
    {
        ...
        if (JsonUtils.hasInteger(obj, "radius"))
        {
            this.radius = JsonUtils.getInteger(obj, "radius");
        }
    }
    ...
    protected void renderSphereBlock()
    {
        ...
        for (int i = 1; i < this.radius + 2; ++i) //130 replaced by this.radius + 2
        {
            // Horizontal rings
    ...
    private void addPositionsOnRing(HashSet<BlockPos> positions, BlockPos.Mutable posMutable, Direction direction)
    {
        if (this.movePositionToRing(posMutable, direction))
        {
            ...
            int failsafe = this.radius * 7; //orgiginally 860
    ...
    private boolean movePositionToRing(BlockPos.Mutable posMutable, Direction dir)
    {
        ...
        int zNext = z;
        int failsafe = 0;
        int failsafeMax = (int)(this.radius * 1.1); //originally 140 when radius was at 128, radius x 1.1 is about that number
    ...
    private boolean isPositionWithinRange(int x, int y, int z)
    {
        ...
        final double maxDistSq = this.radius * this.radius;
